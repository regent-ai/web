defmodule PlatformPhxWeb.HeerichDemoScenesTest do
  ...
  test "raw 0.5 samples expose scaling, carved walls, and restyling" do
    samples = HeerichDemoScenes.samples()
  ...
  test "feature atlas covers the new 0.5 primitives" do
    ...
    assert {"addWhere", _, _} = Enum.find(rows, fn {label, _, _} -> label == "addWhere" end)

    assert {"styleBox / styleLine", _, _} =
             Enum.find(rows, fn {label, _, _} -> label == "styleBox / styleLine" end)
  end
