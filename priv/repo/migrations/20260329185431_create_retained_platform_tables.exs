defmodule PlatformPhx.Repo.Migrations.CreateRetainedPlatformTables do
  ...
  def change do
    create table(:basenames_mint_allowances, primary_key: false) do
    ...
    create index(:basenames_mint_allowances, [:address],
             name: :basenames_mint_allowances_by_address
           )

    create table(:basenames_mints) do
    ...
    create unique_index(:basenames_mints, [:node], name: :basenames_mints_node_unique)

    create unique_index(:basenames_mints, [:payment_tx_hash, :payment_chain_id],
             name: :basenames_mints_payment_tx_unique
           )

    create index(:basenames_mints, [:owner_address], name: :basenames_mints_by_owner)
    create index(:basenames_mints, [:parent_node], name: :basenames_mints_by_parent)

    create table(:basenames_payment_credits) do
      add :parent_node, :string, size: 66, null: false
      add :parent_name, :text, null: false
    ...
    create unique_index(:basenames_payment_credits, [:payment_tx_hash, :payment_chain_id],
             name: :basenames_payment_credits_tx_unique
           )

    create index(:basenames_payment_credits, [:address],
             name: :basenames_payment_credits_by_address
           )

    create index(:basenames_payment_credits, [:parent_node],
             name: :basenames_payment_credits_by_parent
           )

    create table(:agentlaunch_auctions) do
    ...
    create index(:agentlaunch_auctions, [:owner_address], name: :agentlaunch_auctions_by_owner)
    create index(:agentlaunch_auctions, [:status], name: :agentlaunch_auctions_by_status)
    create index(:agentlaunch_auctions, [:ends_at], name: :agentlaunch_auctions_by_ends_at)

    create unique_index(:agentlaunch_auctions, [:network, :auction_address],
             name: :agentlaunch_auctions_by_network_auction_address
           )

    create unique_index(:agentlaunch_auctions, [:source_job_id],
